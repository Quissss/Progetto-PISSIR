...
public class CamSimulatorController : ControllerBase
{
    ...
    {
        _logger.LogInformation("Detected carplate {plate} on arrival", request.CarPlate);
    ...
    private async Task CarDeparting(Request request, Car car)
    {
        if (car.ParkingId != request.ParkingId)
        {
            ...
            _logger.LogWarning("Carplate {plate} is not in parking {parkingId}", request.CarPlate, request.ParkingId);
            return;
        }
